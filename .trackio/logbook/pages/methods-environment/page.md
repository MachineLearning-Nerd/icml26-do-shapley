...
---
<!-- trackio-cell
{"type": "code", "id": "cell_5641bc2d13b8", "created_at": "2026-07-17T04:29:29+00:00", "title": "Pytest suite (28 tests)", "command": ["python", "-m", "pytest", "repro/tests/test_doshapley.py", "-q"], "exit_code": 0, "duration_s": 0.479}
-->
````bash
$ python -m pytest repro/tests/test_doshapley.py -q
````

exit 0 · 0.5s


````python title=test_doshapley.py
...
---
<!-- trackio-cell
{"type": "markdown", "id": "cell_75ade7f73add", "created_at": "2026-07-17T04:33:06+00:00", "title": "Method & environment"}
-->
**Paper:** Parafita et al., "Exactly Computing do-Shapley Values", arXiv 2602.07203, ICML 2026 (OpenReview Peim0KY6ty). **No official code** is released → clean-room implementation from the PDF.

**Environment:** Python 3.12, numpy 2.5, scipy 1.18; 4 vCPU, CPU-only (<1 s tests, ~2 min full evidence).

**Implementation (repro/src):**
- `doshapley.py` — Def. 2.1/2.2 (basis/closure), **Algorithm 1 FindClass**, **Algorithm 2 AllClasses** (closed-set lattice via Lemma 3.1), **Eq. 4** with closed-form weights (Eq. 5), independent **brute-force 2^d Shapley**.
- `scm.py` — linear-Gaussian SCM; faithful do-intervention ν(S)=E[Y|do(S=0)] by graph mutilation + topological propagation; DAG generators sweeping r from ~d+1 (chain) to 2^d.
- `estimator.py` — boundary sampler (Algorithm 3) + value-function-agnostic blind baseline.
- `run_claims.py` — orchestrator → outputs/{c1,c2}_*.csv + summary.json.

**Key correctness fact verified independently:** the do-calculus reduction ν(S)=ν(basis(S))=ν(closure(S)) holds to **0.0** (exact) for every coalition — this is the theorem the entire method relies on, checked rather than assumed.

28/28 pytest tests pass (0.26 s): exact==brute (8 seeds), ν-reduction (6), efficiency (6), symmetry, r-bounds, closed-form-vs-enumeration weights, FindClass canonicality, boundary-exact-at-r, boundary-dominates-blind, 2 negative controls.
